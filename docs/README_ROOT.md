...
⚠️ **Importante**: Você precisa ter PostgreSQL e Redis instalados antes de começar!
...
- **Windows**: Baixe em https://redis.io/download ou use Docker:
  ```bash
  docker run -d -p 6379:6379 redis:latest
  ```
- Redis é usado para filas de processamento assíncrono (logs de auditoria)
...
Crie arquivos `.env` com as seguintes configurações:
...
JWT_SECRET="bombocado-super-secret-jwt-key-2024"
BCRYPT_ROUNDS=12
...
O arquivo `.env` do backend deve ter o mesmo conteúdo que o da raiz.

**Nota**: O NestJS automaticamente carrega ambos os arquivos `.env` (raiz e backend). O Prisma Studio precisa do arquivo `.env` no backend para funcionar corretamente.
...
- ✅ **Sistema de Auditoria** implementado (Logs de atividades)
- ✅ **Frontend** implementado com páginas de Login e Perfil
- ✅ **Componentes UI** reutilizáveis criados
- ✅ **Proteção de Rotas** implementada
- ✅ **Redis/BullMQ** configurado para filas assíncronas
...
- ✅ **Perfil**: Dashboard do usuário com:
  ...
  - Aba de Log de Atividades (histórico completo)
  - Aba de Estatísticas
